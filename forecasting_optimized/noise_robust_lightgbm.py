...
4. Two-stage LightGBM per pipeline:
   ...
   Each pipeline is trained with 3 seeds and averaged; final = 0.5 * v1 + 0.5 * v3, argmax.
...
- unlabeled_pretrain_transactions: not used by this script (only used during analysis to calibrate
  amount/timing tolerances).

Usage
-----
    pip install pandas numpy scikit-learn lightgbm
    python forecasting_optimized/noise_robust_lightgbm.py [--jobs 2] [--threads 2]

`--data-dir` must contain the extracted files (or dataset.zip, which is extracted automatically).
Tested with Python 3.11, pandas 3.0.2, numpy 2.4.4, scikit-learn 1.8.0, lightgbm 4.7.0.
Runtime: ~15-20 min on 2 CPU cores (feature building dominates; use --jobs to parallelise it).
...
FILES = ['train_transactions.jsonl', 'train_labels.csv', 'valid_transactions.jsonl', 'valid_labels.csv',
...
def client_table_v1(Cs):
    T = Cs.groupby('client_id')[V1_CLIENT].first()
...
def client_table_v3(Cs):
    return Cs.groupby('client_id')[V3_CLIENT].first()
...
def fit_predict(version, Ctr, ymap, Cte, seed, threads, rounds_c=400, rounds_m=300):
    ...
    fe = [c for c in Ctr.columns if c not in ('client_id', 'fam', 'target')]
    ctab = client_table_v1 if version == 'v1' else client_table_v3
    lseed = None if version == 'v1' else seed          # v1 uses LightGBM default seeds (as in our runs)
    pc, pm = lgb_params('binary', threads, lseed), lgb_params('multi', threads, lseed)
    ...
    for a, _ in StratifiedKFold(5, shuffle=True, random_state=seed).split(ug, gy):
        ...
        m = lgb.train(pc, lgb.Dataset(Ctr.loc[inA, fe], Ctr.loc[inA, 'target']), rounds_c)
        oof.loc[~inA] = m.predict(Ctr.loc[~inA, fe])
    mc = lgb.train(pc, lgb.Dataset(Ctr[fe], Ctr.target), rounds_c)
    ...
    mm = lgb.train(pm, lgb.Dataset(Xtr, yy.map(LABELS.index)), rounds_m)
...
def main():
    ...
    artifact_dir = os.path.join(repository, 'forecasting_optimized', 'artifacts')
    ap = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    ap.add_argument('--data-dir', default=os.path.join(repository, 'data', 'dataset'))
    ap.add_argument('--out', default=os.path.join(artifact_dir, 'lightgbm_submission.csv'))
    ap.add_argument('--prob-dir', default=os.path.join(artifact_dir, 'lightgbm'))
    ...
    ap.add_argument('--cache-dir', default=os.path.join(artifact_dir, 'lightgbm_feature_cache'), help="'' disables caching of feature tables")
    ...
    # ---- data: train + 5 noisy copies (training), valid (scoring only), test (submission)
    log('loading + tagging')
    ...
    datasets += [('valid', tag(load(args.data_dir, 'valid'))), ('test', tag(load(args.data_dir, 'test')))]
    ...
    # ---- models: train on train (+ copies) only
    probs = {}
    for vi, version in enumerate(['v1', 'v3']):
        Ctr = pd.concat([feats[n][vi] for n in train_names], ignore_index=True)
        Ctr['target'] = (Ctr.fam.values == Ctr.client_id.map(lambda c: ymap.get(base_id(c))).values).astype(int)
        Cev = pd.concat([feats['valid'][vi], feats['test'][vi]], ignore_index=True)
        Ps = []
        for seed in seeds:
            Ps.append(fit_predict(version, Ctr, ymap, Cev, seed, args.threads)); log(f'  {version} seed {seed} done')
        probs[version] = sum(Ps) / len(Ps)
    blend = 0.5 * probs['v1'][LABELS] + 0.5 * probs['v3'][LABELS]

    os.makedirs(args.prob_dir, exist_ok=True)
    for split, client_ids in [('valid', yva.index), ('test', ss.client_id)]:
        np.savez_compressed(
            os.path.join(args.prob_dir, f'{split}_probabilities.npz'),
            client_ids=np.asarray(client_ids, dtype=str),
            probabilities=blend.loc[client_ids, LABELS].to_numpy(dtype=np.float64),
        )

    # ---- validation scores (valid never used for training)
    def pred(P, ids): return np.array(LABELS)[P.loc[ids][LABELS].values.argmax(1)]
    print('\nMacro-F1 on validation (1,000 clients, not used for training):')
    for nm, P in [('v1', probs['v1']), ('v3', probs['v3']), ('blend (submitted)', blend)]:
        print(f'  {nm:18s} {f1_score(yva.values, pred(P, yva.index), average="macro"):.4f}')
    print(classification_report(yva.values, pred(blend, yva.index), digits=3))

    # ---- submission
    sub = pd.DataFrame({'client_id': ss.client_id, 'predicted_next_recurring_merchant': pred(blend, ss.client_id)})
